...
export const AlertRowActions: React.FC<AlertRowActionsProps> = ({ 
    ...
    return (
        <div className="flex items-center gap-1">
            {row.state !== 'read' && (
                <Tooltip title={t("portfolio.alerts.mark_read")}>
                    <IconButton size="small" onClick={() => onMarkAsRead(row.id)}>
                        <CheckCircle className="h-4 w-4 text-gray-400 hover:text-green-600" />
                    </IconButton>
                </Tooltip>
            )}
            <Tooltip title={row.snoozed_until ? t("portfolio.alerts.unsnooze") : t("portfolio.alerts.snooze_24h")}>
            ...
            <Tooltip title={t("portfolio.alerts.delete")}>
