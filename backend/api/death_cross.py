...
def load_existing_death_cross_analysis(
    db, market_ids, companies, short_window, long_window
):
    existing = (
        db.query(AnalysisResult)
        .filter(AnalysisResult.analysis_type == "death_cross")
...
def cached_death_cross(
    request: DeathCrossRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """
    Checks for a "death cross" using the AnalysisResult cache.
    ...
    analysis_map = load_existing_death_cross_analysis(
        db, market_ids, companies, short_window, long_window
    )
    logger.info(
    ...
    analyze_and_build_results(
        ...
        min_volume=min_volume,
        adjusted=adjusted,
        death_cross_results=death_cross_results,
    )

    elapsed = time.time() - start_time
    logger.info(
        f"Death‐cross checked {len(companies)} companies "
        f"({len(pairs_to_check)} fresh) in {elapsed:.2f}s"
    )

    if not death_cross_results:
        return {"status": "success", "data": []}
    death_cross_results.sort(key=lambda x: x["ticker"])
    return {"status": "success", "data": death_cross_results}
