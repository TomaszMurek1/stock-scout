...
@router.post("/buy")
def buy_stock(req: TradeRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    company = db.query(Company).get(req.company_id)
    if not company:
        raise HTTPException(status_code=404, detail="Company not found")

    portfolio = db.query(Portfolio).filter_by(user_id=user.id).first()  # for now, just use default

    pos = db.query(PortfolioPosition).filter_by(portfolio_id=portfolio.id, company_id=req.company_id).first()
    if not pos:
...
@router.post("/sell")
def sell_stock(req: TradeRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def get_positions(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    for pos in positions:
        current_price = pos.company.market_data.current_price if pos.company.market_data else None
